import { showDrawer, bulkDialog } from "./admin-dialogs.js";
    ...
    .forEach((wrap, index) => {
          ...
          const source =
            key === "requests"
              ...
              : key
                ...
                : [];
          const record = source.find((x) => x.id === id) || {
            id,
      ...
      for (const entry of entries) {
        ...
        if (entry.key) {
          ...
          quick.type = "button";
          quick.className = "admin-quick-detail";
          quick.setAttribute(
            "aria-label",
            (entry.record.title || entry.record.name || entry.id) +
              " 빠른 상세",
          );
          quick.innerHTML = icon("file-search-line");
          entry.row.lastElementChild.prepend(quick);
          quick.onclick = () => showDrawer(entry.key, entry.record, quick);
        }
        check.onchange = () => {
      ...
      [...head.cells].slice(1).forEach((cell, offset) => {
        ...
        option.querySelector("input").onchange = (e) => {
          ...
          r.saveAdminPreference("table:" + location.pathname + ":" + index, {
            hidden: [...hidden],
      ...
      bar.querySelector("[data-export]").onclick = () => {
        ...
        a.href = objectURL;
        a.download = "kowsc-selected.csv";
        a.click();
        setTimeout(() => URL.revokeObjectURL(objectURL), 1000);
        toast(`${selected.length}건 내보내기 완료`);
      };
      bar.querySelector("[data-bulk]").hidden = !entries.some(
        (e) => e.key && e.key !== "notificationTemplates",
      );
      bar.querySelector("[data-bulk]").onclick = () => {
        const chosen = entries.filter((e) => selection.ids.has(e.id));
        if (chosen.length)
          bulkDialog(
            chosen[0].key,
            chosen.map((e) => e.record),
            bar.querySelector("[data-bulk]"),
            () => location.reload(),
          );
      };
      render();
